<template>
  <div class="panelWrapper">
    ...
    <div class="body">
      <div class="left">
        <div class="category" v-for="item in allPlaylistCategory" :key="item.name">
          <i class="sprite_icon2 img" :style="getBackgroundPosition(item.name)" />
          <span>{{ item.name }}</span>
        </div>
      </div>
      ...
      <div class="right">
        <ul class="row" v-for="item in allPlaylistCategory" :key="item.name">
          <li class="subItem" v-for="subItem in item.subs" :key="subItem.name">
            <span class="category-subItem" @click="changeCatClick(subItem.name)">{{
              subItem.name
            }}</span>
            <span>|</span>
          </li>
...
const allPlaylistCategory = computed(() => store.state.playlist.allPlaylistCategory);

const changeCatClick = (catName: string) => {
  store.dispatch('changeCurrentCatAction', catName);
  store.dispatch('getAllPlaylistAction', { offset: 1, limit: 50, cat: catName });
