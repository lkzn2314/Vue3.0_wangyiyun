<template>
  <div
    :class="['playbar-wrapper', ' playbar_sprite', isFixedPos ? 'fixed-position' : '']"
    :style="{ bottom: isShowBar ? '0' : '-40px' }"
  >
    <div class="content wrap-v2">
      <div class="control">
        <div class="prev playbar_sprite" />
        <div
          class="play playbar_sprite"
          :style="{ backgroundPositionY: isPlaying ? '-165px' : '-204px' }"
          @click="playMusic"
        />
      ...
      <div class="play-info">
            ...
            :src="
              formatImgSize(
                currentSong?.al?.picUrl ||
                  'http://s4.music.126.net/style/web2/img/default/default_album.jpg?limit=34x34',
                34,
              )
            "
            alt=""
        ...
        <div class="info">
          ...
          <div class="progress">
            ...
            <span class="time">
              <span class="now-time">{{ moment(currentTime).format('mm:ss') }}</span>
              <span class="divider">/</span>
              <span class="all-time">{{ moment(allTime).format('mm:ss') }}</span>
            </span>
          </div>
        </div>
      </div>
    ...
    <div class="lock-wrapper playbar_sprite">
      <i
        ...
        :style="{ backgroundPositionX: isFixedPos ? '-100px' : '-80px' }"
        @click="changeLockClick"
      />
    </div>

    <audio ref="audioRef" @timeupdate="(e) => timeUpdate(e)" @ended="handleMusicEnd" />
  </div>
</template>
...
export default {
  ...
  setup() {
    ...
    watch(
      () => currentSong.value,
      (newSong: { [key: string]: any }) => {
        allTime.value = newSong.dt || 0;
        audioRef.value.src = getSongPlay(newSong.id);
    ...
    const timeUpdate = (e: any) => {
      ...
      progress.value = (currentTime.value / allTime.value) * 100;
    };
